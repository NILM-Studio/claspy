...
2.  **自适应均值回归与异常值处理 (Step 2)**:    
    ...
    *   **滑动窗口异常值去除**: 
        - **算法原理**: 采用基于滚动中位数的 Z-score 方法检测异常值，比传统基于均值的方法更鲁棒
        - **窗口大小**: 默认使用 20 个数据点的滑动窗口
        - **阈值设置**: Z-score 阈值默认设置为 3.0（可调整）
        - **检测流程**:
            1.  计算每个数据点周围滑动窗口内的中位数和标准差
            2.  计算 Z-score: `z = (x - median) / std`
            3.  当 |z| > 阈值时，判定为异常值
        - **处理方式**: 支持多种插值方法处理异常值
            - `linear`: 线性插值（默认）
            - `polynomial`: 多项式插值（2阶）
            ...
            - `nearest`: 最近邻插值
            - `zero`: 零阶插值
        - **结果标记**: 添加 `is_interpolated` 列标记插值点 (True/False)
        - **列名调整**:
            - `power_origin`: 原始功率数据
            - `power`: 经过异常值去除后的干净数据
    *   **Score > 阈值**: 判定为多状态数据：
